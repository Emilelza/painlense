...
def post_patient(request: CreatePatientRequest, _patient_id: str = Depends(require_token)):
    try:
        patient_id = request.patient_id.strip()
        if not patient_id:
            return JSONResponse(status_code=400, content={"error": "Patient ID is required"})
        if not supabase_store.enabled() and local_patient_login_exists(patient_id):
